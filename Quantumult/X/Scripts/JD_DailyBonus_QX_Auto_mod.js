  ...
  setTimeout(function() {
    ...
    $nobyda.post(JRDSUrl, function(error, response, data) {
      if (error) {
        ...
        notice(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign)
      } else {
        const cc = JSON.parse(data)
        if (data.match(/京豆X/)) {
...
function notice(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign) {
  $nobyda.notify(JRDSign, JDBean, JRBean + JDturn + JRSteel + JDShake1 + JDShake2 + JDShake3)
  $nobyda.end()
}
