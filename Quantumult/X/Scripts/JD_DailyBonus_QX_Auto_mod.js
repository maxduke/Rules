...
function JingDongShake1(JDBean, JDturn, JRBean, JRSteel) {
    setTimeout(function() {
      const JDSh = {
        url: 'https://api.m.jd.com/client.action?appid=vip_h5&functionId=vvipclub_shaking',
        headers: {
      ...
      $nobyda.get(JDSh, function(error, response, data) {
        if (error) {
          const JDShake1 = "京东商城-摇摇: 签到接口请求失败 ‼️‼️\n" + error
          JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
        } else {
          const cc = JSON.parse(data)
          if (data.match(/prize/)) {
            if (log) console.log("京东商城-摇一摇签到成功response: \n" + data)
            if (cc.data.prizeBean) {
              const JDShake1 = "京东商城-摇摇: 签到成功, 明细: " + cc.data.prizeBean.count + "京豆 🐶"
              JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
            } else {
              if (cc.data.prizeCoupon) {
                const JDShake1 = "京东商城-摇摇: 获得满" + cc.data.prizeCoupon.quota + "减" + cc.data.prizeCoupon.discount + "优惠券→ " + cc.data.prizeCoupon.limitStr
                JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
              } else {
                const JDShake1 = "京东商城-摇摇: 需修正‼️日志发至TG:@NobyDa_bot"
                JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
              }
            }
          } else {
            if (log) console.log("京东商城-摇一摇签到失败response: \n" + data)
            if (data.match(/true/)) {
              const JDShake1 = "京东商城-摇摇: 签到成功, 明细: 无奖励 🐶"
              JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
            } else {
              if (data.match(/(无免费|8000005)/)) {
                const JDShake1 = "京东商城-摇摇: 签到失败, 原因: 已摇过 ⚠️"
                JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
              } else if (data.match(/(未登录|101)/)) {
                const JDShake1 = "京东商城-摇摇: 签到失败, 原因: Cookie失效‼️"
                JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
              } else {
                const JDShake1 = "京东商城-摇摇: 需修正‼️日志发至TG:@NobyDa_bot"
                JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1)
  ...
  function JingDongShake2(JDBean, JDturn, JRBean, JRSteel, JDShake1) {
    setTimeout(function() {
      const JDSh = {
        url: 'https://api.m.jd.com/client.action?appid=vip_h5&functionId=vvipclub_shaking',
        headers: {
      ...
      $nobyda.get(JDSh, function(error, response, data) {
        if (error) {
          const JDShake2 = "京东商城-摇摇: 签到接口请求失败 ‼️‼️\n" + error
          JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
        } else {
          const cc = JSON.parse(data)
          if (data.match(/prize/)) {
            if (log) console.log("京东商城-摇一摇签到成功response: \n" + data)
            if (cc.data.prizeBean) {
              const JDShake2 = "京东商城-摇摇: 签到成功, 明细: " + cc.data.prizeBean.count + "京豆 🐶"
              JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
            } else {
              if (cc.data.prizeCoupon) {
                const JDShake2 = "京东商城-摇摇: 获得满" + cc.data.prizeCoupon.quota + "减" + cc.data.prizeCoupon.discount + "优惠券→ " + cc.data.prizeCoupon.limitStr
                JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
              } else {
                const JDShake2 = "京东商城-摇摇: 需修正‼️日志发至TG:@NobyDa_bot"
                JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
              }
            }
          } else {
            if (log) console.log("京东商城-摇一摇签到失败response: \n" + data)
            if (data.match(/true/)) {
              const JDShake2 = "京东商城-摇摇: 签到成功, 明细: 无奖励 🐶"
              JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
            } else {
              if (data.match(/(无免费|8000005)/)) {
                const JDShake2 = "京东商城-摇摇: 签到失败, 原因: 已摇过 ⚠️"
                JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
              } else if (data.match(/(未登录|101)/)) {
                const JDShake2 = "京东商城-摇摇: 签到失败, 原因: Cookie失效‼️"
                JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
              } else {
                const JDShake2 = "京东商城-摇摇: 需修正‼️日志发至TG:@NobyDa_bot"
                JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2)
  ...
  function JingDongShake3(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2) {
    setTimeout(function() {
      const JDSh = {
        url: 'https://api.m.jd.com/client.action?appid=vip_h5&functionId=vvipclub_shaking',
        headers: {
      ...
      $nobyda.get(JDSh, function(error, response, data) {
        if (error) {
          const JDShake3 = "京东商城-摇摇: 签到接口请求失败 ‼️‼️\n" + error
          ...
          if (data.match(/prize/)) {
            if (log) console.log("京东商城-摇一摇签到成功response: \n" + data)
            if (cc.data.prizeBean) {
              const JDShake3 = "京东商城-摇摇: 签到成功, 明细: " + cc.data.prizeBean.count + "京豆 🐶"
              JRDoubleSign(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3)
            } else {
              if (cc.data.prizeCoupon) {
                const JDShake3 = "京东商城-摇摇: 获得满" + cc.data.prizeCoupon.quota + "减" + cc.data.prizeCoupon.discount + "优惠券→ " + cc.data.prizeCoupon.limitStr
                JRDoubleSign(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3)
              } else {
                const JDShake3 = "京东商城-摇摇: 需修正‼️日志发至TG:@NobyDa_bot"
                JRDoubleSign(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3)
              }
            }
          } else {
            if (log) console.log("京东商城-摇一摇签到失败response: \n" + data)
            if (data.match(/true/)) {
              const JDShake3 = "京东商城-摇摇: 签到成功, 明细: 无奖励 🐶"
              JRDoubleSign(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3)
            } else {
              if (data.match(/(无免费|8000005)/)) {
...
function JRDoubleSign(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3) {
  setTimeout(function() {
    const JRDSUrl = {
    ...
    $nobyda.post(JRDSUrl, function(error, response, data) {
      if (error) {
        ...
        JingRongLottery(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign)
      } else {
        const cc = JSON.parse(data)
        if (data.match(/京豆X/)) {
...
function JingRongLottery(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign) {
  const JRLUrl = {
  ...
  $nobyda.get(JRLUrl, function(error, response, data) {
    if (error) {
      ...
      notice(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign, JRLottery)
    } else {
      const cc = JSON.parse(data)
...
function notice(JDBean, JDturn, JRBean, JRSteel, JDShake1, JDShake2, JDShake3, JRDSign, JRLottery) {
  $nobyda.notify(JRDSign, JDBean, JRBean + JDturn + JRSteel + JDShake1 + JDShake2 + JDShake3 + JRLottery)
}
